...
A prefrozen candidate-level calibration using only existing ranker signals initially looked strong on a concept-separated synthetic evaluation split: it retained **158/159** baseline Hit@5 targets and **30/30** targets originally at ranks 2–5 while reducing exact-target-negative visible candidates **38.3%** and mean list length `4.98 -> 3.40`. However, after the rule was frozen, the opened 54-case replay showed unsafe transfer: targetable Hit@5 moved **28/41 -> 22/41** while mean list length moved `4.59 -> 3.30`.

Decision: **do not deploy the v0 display gate and do not use a fixed top-N cutoff.** Hit@5 retention, especially ranks 2–5, remains a hard safety requirement. The next bounded parallel experiment may keep the A ranker unchanged and test whether separately preserved canonical-vs-teacher/diversified query-candidate evidence gives a more stable relevance signal. This is experimental only; do not activate a production reranker merely to make lists shorter. Human relevance validation remains the eventual decision-bearing gate.

Evidence: `docs/findings/p80-display-relevance-calibration-2026-09-09.md`, `research/evaluation/v31/p80-display-precision-opened-audit-v0.json`, `research/evaluation/v31/p80-display-relevance-calibration-v0.json`, and `research/evaluation/v31/p80-display-relevance-opened-replay-v0.json`.
