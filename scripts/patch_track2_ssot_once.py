...
old = '''### 2.1.1 Two-track research order — Track 1 first
...
'''
new = '''### 2.1.1 Two-track research order — Track 2 active; Track 1 parked
...
'''
if old not in s:
    raise SystemExit('track anchor not found')
s = s.replace(old, new, 1)

old = '''Complexity has a burden of proof. A new source, retrieval lane, model, reranker or deployment component is admitted only if it produces a material measured gain on the Pareto view or fixes a defined safety/regression failure. If two configurations perform similarly, prefer the one with fewer sources, rules, models and moving parts.
'''
new = old + '''
...
'''
if old not in s:
    raise SystemExit('complexity anchor not found')
s = s.replace(old, new, 1)

old = '''Current deployment principle:
...
'''
new = '''Current deployment constraint:
