...
This is consistent with the Track-2 semantic-description hypothesis: free text is an input to mapping, not a fabricated taxonomy identity.
...
The 2026-09-06 plan parked broad Track-1 expansion and stopped semantic retrieval micro-optimisation until new independent human/user evidence or a measured product failure appeared.

This feedback satisfies that trigger. **Track 1 is reopened only as a bounded field-feedback replay**, while Track 2 remains the semantic-description capability track.
...
7. **Use residual description-language failures to evaluate Track 2.** A case becomes Track-2 evidence only after ordinary canonical lookup/alias/context handling is shown insufficient.
...
- `CANONICAL_CONCEPT_GAP` — no adequate active taxonomy identity found;
- `TRACK2_DESCRIPTION_RESIDUAL` — ordinary structured discovery is insufficient but a canonical target exists;
