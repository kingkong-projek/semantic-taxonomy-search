...
Current Track-2 result: the earlier YV C2 evidence is retained as a bounded P80/router regression result, but **the occupation-description candidate is now frozen as `YV-description-full-v0-canonical-router`**: all **2,105** active v31 `occupation-name` identities ranked from canonical preferred label + real definition + canonical alternative labels, plus the existing privileged exact active job-title→typed occupation-parent route. On the frozen 333-case YV source-truth suite, expanding the same ranker from 165 to 2,105 candidates preserves **333/333 Top1, 333/333 Hit@5 and MRR 1.0 with zero rank changes**. On a strict leak-free, source-attested but already-opened 17-case Yrkesinformation `work_task` diagnostic, full canonical retrieval reaches **9/17 Hit@5** versus **1/17** for the old 165-target universe. Separate AF ad-language and Relevanta-kompetenser lanes are genuinely complementary (oracle union **15/17**), but fixed slots, one concatenated BM25 document and standard equal-weight RRF failed to preserve that complementarity without ranking damage; they are therefore **not promoted**. Ad/skill context remains diagnostic candidate evidence for future independent residuals, not active runtime fusion. C2 remains a regression/demo reference. Evidence: `docs/findings/yv-full-universe-description-baseline-v31.md` and `research/evaluation/v31/yv-full-universe-description-baseline.json`. For skills, **`KV-G1-4slot` remains the independently source-attested base**, while the strongest current low-complexity research candidate remains **`KV-G1+T3`**: preserve canonical C0 rank 1, admit one unique candidate from the existing G1 BM25 lane, then admit three unique candidates from a second BM25 view containing the same G1 evidence plus three frozen teacher phrases per P80 skill. Runtime still uses no language model, embeddings/vector index, learned reranker, score calibration or query classifier.
...
**Compile-time semantic tournament (ACTIVE next architecture phase; 2026-09-07).** The target universe is small and mostly static, so the next architecture question is not `which neural model can the browser tolerate?` but **how much semantic intelligence can be paid once at build time and compiled into a tiny deterministic runtime artifact**. **Teacher/build marginal-cost budget is at most 10 SEK total for the current tournament phase, with zero-cost access preferred. No local teacher inference is available.** The currently allowed teacher pool is therefore limited to **Gemma 4 through the already-available zero-cost hosted access as the primary bulk teacher**, **Muse Spark as a zero/very-low-cost fallback or independent challenger if needed**, and **GPT-5.6 Sol in the existing ChatGPT conversation** for bounded prompt design/judging. Do not exceed 10 SEK cumulative marginal API spend without an explicit new project decision. The browser must not inherit teacher parameter count or runtime dependency.
...
**Tournament scoreboard update (2026-09-08).** Round 0 falsified three lower-bound formulations without eliminating their broader architecture classes: B0 token->concept sparse E5 geometry was only **117,908 bytes gzip** but scored **0/17 YV and 0/81 KV Hit@5**; C0 PCA32/int8 mean-token student was **712,990 bytes gzip** but scored **0/17 YV and 6/81 KV**; D0 Model2Vec was **16.8 MB gzip** and scored only **3/17 YV and 20/81 KV**, so it fails both the client-size and quality bars. The useful teacher signal is therefore treated as sequence/context-level rather than token geometry. Evidence: `research/evaluation/v31/compile-time-semantic-tournament-round0.json`.

**First A result — `YV-A149-Gemma4-26B-sequence-expansion-v0` (2026-09-08): CONTINUE A.** A bounded zero-cost Gemma 4 26B run generated **1,192 Swedish teacher descriptions for 149/2,105 occupations** (149/150 requests succeeded). Appending those phrases as ordinary BM25 document language, never as exact identity surfaces, improves the already-opened 40 targetable YV stress cases from **18/40 -> 24/40 Hit@5** and **9/40 -> 12/40 Top1**: direct **8/12 -> 10/12**, colloquial **2/12 -> 3/12**, noisy **5/8 -> 7/8**, indirect **3/8 -> 4/8**. Only **20/40** stress target families and **2/17** strict source-attested targets are covered by the partial teacher corpus; accordingly the strict 17-case Hit@5 remains **9/17** while MRR moves **0.418849 -> 0.443722**. The compact partial teacher map is only **18,103 bytes gzip-9**; a linear full-universe projection is about **256 kB gzip**, but that projection is not evidence until the full artifact is generated and measured. Negative/abstention behavior did not improve. A raw single-ranker replacement moves the 333 canonical source-truth suite from **333/333 -> 330/333 Top1** while preserving **333/333 Hit@5**; this reinforces rather than changes the product boundary: **ordinary YV/KV lookup remains the unchanged privileged Track-1 path, while A belongs only to the explicit description-fallback entrypoint**. Do not infer the UI lane from query wording. Execution order after A149 is now explicit. First use a deterministic coverage checkpoint around **A600** (A149 plus the next roughly 450 identities in the frozen order) and replay the same frozen diagnostics unchanged; this checkpoint tests whether A149's gain survives materially broader coverage and is not a tuning gate. If the signal holds, continue Gemma teacher generation across **all 2,105 active YV occupation identities** so missing teacher coverage is no longer a confounder. Full teacher coverage is an **offline evidence corpus**, not a requirement that every generated phrase must ship to the browser. The first full-coverage falsification is deliberately simple: build and measure an **unpruned A2105** description-lane candidate, including actual compressed bytes and retrieval-noise/regression behavior. Only after that may runtime compilation prune or down-weight low-value long-tail language, and such pruning must not be tuned on the opened 17/88 evaluation queries. Hard-negative mining likewise comes **after broad/full positive coverage**, so remaining near-neighbour confusions can be distinguished from mere missing coverage; mine from taxonomy/source neighbours plus retrieval errors on teacher-generated/public training language, never from opened evaluation queries. Do not pre-build chunking/sharding: introduce it only if measured transfer/init/runtime evidence justifies the extra requests and cache/routing complexity. The current ~256 kB full-universe teacher-map projection is only a projection, but it gives no present reason to shard. Evidence: `research/evaluation/v31/compile-time-semantic-tournament-a149.json`.


**A593 coverage checkpoint — `YV-A593-Gemma4-26B-sequence-expansion-v0` (2026-09-08): CONTINUE A TO FULL COVERAGE, BUT COVERAGE ALONE IS NOT ENOUGH.** The frozen A149 corpus and the successful portion of the next 450-request batch merge to **593 unique occupation identities / 4,744 teacher phrases**; the checkpoint is A593 rather than A600 because the original pilot had one invalid response and the new batch had six format-invalid 7/9-phrase responses. The compact teacher map is **68,317 bytes gzip-9** (217,332 raw), with a still-only-linear full-2,105 projection of **242,508 bytes gzip**. On the same already-opened 40 targetable YV stress cases, baseline -> A149 -> A593 is **18/40 -> 24/40 -> 26/40 Hit@5** and **9/40 -> 12/40 -> 15/40 Top1**. A593 category Hit@5 is direct **12/12**, colloquial **3/12**, noisy **7/8**, indirect **4/8**. Teacher family coverage has meanwhile risen to **34/40** targetable cases (direct 11/12, colloquial 11/12, noisy 6/8, indirect 6/8), versus 20/40 at A149. Thus fourteen additional covered stress families buy only two additional Hit@5 successes, both in the direct stratum; colloquial/noisy/indirect Hit@5 do not improve from A149. This is the first clear evidence that the remaining hard residual is increasingly a **discrimination/ranking problem, not merely missing positive teacher coverage**. On the strict source-attested 17, A593 reaches **10/17 Hit@5, 5/17 Top1, MRR 0.424510** versus A149 **9/17, 6/17, 0.443722** and baseline **9/17, 6/17, 0.418849**: recall improves by one while rank quality adds noise. Canonical source-truth remains **333/333 Hit@5** and **331/333 Top1** in the raw description ranker; ordinary Track-1 lookup remains separate and unchanged. Negative/abstention buckets remain unchanged. **Execution decision after qualitative A593 review:** **PAUSE A2105/full Gemma generation. Work the existing A593 corpus first.** The opened stress replay is only a coarse architecture diagnostic and its current `family_rank` substring heuristic can both over-credit and under-credit results (for example, `Djurfysioterapeut` can satisfy `fysioterapeut`, while an excellent `Backend-utvecklare` result can miss the expected `systemutvecklare/programmerare` family). Qualitative inspection of A593 shows several mechanisms that can be tested without any new teacher calls: (1) flattening all eight teacher phrases into one BM25 document lets unrelated generic tokens accumulate across phrases; (2) colloquial generated tokens such as `står`, `fyller`, `går runt`, `hjälper`, `hitta rätt`, `kollar` and `ser till` can receive disproportionate sparse weight and attract unrelated concepts; (3) exact-token morphology leaves useful bridges unused (`katten/sjuk` versus `katter/sjuka`); (4) several remaining indirect queries need discrimination/paraphrase evidence rather than more concepts; and (5) abstention is still a separate unresolved problem. Next use only the frozen 593 concepts / 4,744 phrases to run corpus-internal, non-evaluation-leaking diagnostics: phrase-level versus flattened scoring, leave-one-phrase-out/self-retrieval, confusion-pair/hard-negative mining, generic-token/phrase analysis and the smallest morphology-normalization probes. The opened 17/88 suites may be replayed afterward only as diagnostics, never used to choose per-query fixes. Resume broad/full Gemma generation only if these A593 experiments show that missing concept coverage remains a material residual. Chunking remains unjustified by measured bytes. Evidence: `research/evaluation/v31/compile-time-semantic-tournament-a593.json`.

**A593 representation/morphology diagnostic (2026-09-08): KEEP FLATTENED PHRASES; SNOWBALL IS PROMISING BUT NOT PROMOTED.** Using only the frozen **593 concepts / 4,744 Gemma phrases**, an 8-fold leave-one-phrase-slot-out diagnostic compared the current flattened BM25 representation with per-phrase max scoring and deterministic Swedish Snowball stemming. The per-phrase max variant is decisively worse than flattened raw BM25 (**Top1 13.98% vs 19.01%; Hit@5 31.75% vs 37.73%**) and is rejected. Swedish Snowball applied only inside the description candidate's lexical BM25 improves the corpus-internal holdout to **22.11% Top1 and 43.40% Hit@5**; paired versus flattened raw it creates **253 Top1 gains / 106 losses** and **360 Hit@5 gains / 91 losses**. The same candidate was selected without using opened evaluation queries and then replayed once on the opened diagnostics: targetable YV stress improves from raw A593 **26/40 -> 30/40 Hit@5** and **15/40 -> 18/40 Top1**; category Hit@5 becomes direct **12/12**, colloquial **6/12**, noisy **8/8**, indirect **4/8**. However the strict source-attested 17 regresses from raw A593 **10/17 -> 9/17 Hit@5**, **5/17 -> 4/17 Top1**, and **MRR 0.424510 -> 0.390739**. Canonical source-truth remains **333/333 Hit@5** and improves raw-description Top1 **331/333 -> 332/333**, while abstention remains unchanged. Therefore Snowball is **mechanistic evidence, not a promoted runtime change**. It confirms that morphology is a material part of the residual but also that a global stemming change shifts ranking trade-offs. Keep A2105 paused. Next exploit the A593 self-retrieval corpus to mine false-positive hubs/confusion pairs and test the smallest provenance-preserving weighting/penalty or hard-negative mechanism; do not hand-tune opened 17/88 queries. The corpus-internal diagnostic already exposes strong hubs, especially `SFI-lärare` (**192 false Top1 wins raw; 169 with Snowball**), so hubness/generic lexical evidence is now a concrete next target. Evidence: `research/evaluation/v31/compile-time-semantic-a593-phrase-representation.json` and `research/evaluation/v31/compile-time-semantic-tournament-a593-snowball.json`.


**A593 sparse paper-note / hard-negative checkpoint (2026-09-08): KEEP AS ARCHITECTURE EVIDENCE; DO NOT PROMOTE YET.** The literal paper-note formulation `stemmed-token stability x concept-IDF` is roughly Snowball-level on the frozen 4,744 Gemma phrase-slot holdouts (**22.53% Top1 / 43.09% Hit@5**) and does not justify replacing BM25. A stronger compile-time distillation using **word-boundary character 3/4-gram TF-IDF centroids**, pruned to **300 features per occupation** and globally quantized to uint8, reaches **31.94% Top1 / 61.72% Hit@5 / MRR 0.4535** on those same outer holdouts. The full A593 binary artifact is **560,242 bytes raw / 451,128 bytes gzip-9**, with 10,676 features and 169,061 non-zero weights; runtime is only character n-gram extraction, TF-IDF and sparse dot products, with no teacher/model dependency. However the prefrozen standalone 593-occupation semantic lane transfers much less strongly to the already-opened YV stress replay: **21/40 Hit@5 / 15/40 Top1**, with direct **11/12 Hit@5** but colloquial **3/12**, noisy **5/8** and indirect **2/8**. It abstains on none of the negative stress cases. This is evidence of a **teacher-distribution/generalization gap**: excellent model-authored self-retrieval is not sufficient evidence of user-language quality. Do not tune top-K/features from the opened replay.
...
1. **A — teacher/doc2query-expanded sparse BM25.** Use large teachers offline to generate diverse Swedish user-language descriptions for broad/full canonical coverage (task language, colloquial paraphrases, tools/methods, misspellings/noise and indirect narrative forms). After coverage is no longer the confounder, mine hard-negative/confusable evidence from taxonomy/source neighbours and corpus-internal retrieval errors, then prune/compile only language that earns its quality/byte cost into provenance-separated sparse documents. Runtime remains tokenization + sparse scoring; no model ships to the client.
...
Teacher diversity is desirable, but the executable pool is deliberately narrow under the **<=10 SEK total / no-local-inference constraint**. Gemma 4 is the primary bulk teacher because the available hosted tier is zero-cost; Muse Spark is reserved for cases where an independent teacher is materially useful and must stay within the same total budget; GPT-5.6 Sol in ChatGPT remains the high-capability interactive teacher/judge for bounded batches, prompt design, difficult boundary cases and corpus quality review. Do not assume Sol is a reproducible batch API. Do not add another paid or local teacher merely for ensemble diversity. Teacher disagreement must be retained as evidence rather than silently collapsed into false ground truth. Source-attested taxonomy/AF evidence remains provenance-distinct from generated teacher language. Any external teacher input must be public or synthetic by construction; user logs, personal data and unpublished sensitive material are excluded from this build path.
...
**Human-gate status (2026-09-07): candidate-construction gates are cleared for both streams; collection has not started.** The occupation candidate is now frozen as `YV-description-full-v0-canonical-router`, and the structured contract treats **all 2,105 active v31 occupation-name identities** as the occupation capability universe; historical P80 membership is a demand-priority stratum, not an occupation eligibility boundary. The skill candidate remains plain `KV-G1+T3`. The no-example participant prompt, blind adjudication rules, staged case validator, preregistration template, preregistration→manifest SHA binding, raw-text-free need/prevalence funnel format and repository stage-order guard are in place. No real human description, adjudication or outcome file is present in the repository yet. **Neither stream may begin collection from the template state:** before the first participant in any enabled stream, `preregistration.json` must replace all placeholders, satisfy the approved recruitment/data-handling requirements and be committed with status `frozen-before-first-participant`; retrieval may run only after elicitation and blind adjudication are frozen and a manifest binds all three SHA-256 values. A recruited capability study must not be reported as production prevalence without the separate eligible lexical-failure denominator.
...
The semantic feature is **not required to have equally strong evidence or ranking quality across the full taxonomy long tail in v0**, and P80 remains the demand-priority research stratum. But a research envelope must not silently redefine a valid canonical identity as impossible. For YV description capability, all active v31 `occupation-name` identities remain eligible destinations and confidence/abstention must be distinguished from candidate-universe exclusion. The existing lexical picker remains the complete deterministic product path; semantic retrieval may abstain when evidence is weak, but not because an otherwise valid occupation was omitted from the candidate universe by the old P80 prototype.
...
**v0 semantic-lane correction (2026-09-07):** P80 remains the demand-priority benchmark stratum, **not a hard YV occupation-description candidate boundary**. The Track-2 YV capability must be able to return any active v31 `occupation-name` identity (2,105); confidence may still fail closed on weak evidence. Published job titles remain fully available in the ordinary lexical picker and may be semantic routing/context vocabulary, but direct semantic `job-title` destinations remain out of the core occupation target space. KV semantic description research continues to use the P80 skill set until separate skill evidence justifies expansion.
...
- the **159 P80 occupation-name identities** remain the demand-priority benchmark core, but any independently adjudicated valid active v31 `occupation-name` outside P80 remains capability-eligible; P80 membership cannot make a valid occupation unmappable;
